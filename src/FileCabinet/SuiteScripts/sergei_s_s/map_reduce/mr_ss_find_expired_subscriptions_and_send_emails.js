 ...
 */
    (email, record, runtime, search) => {
        ...
        const map = (mapContext) => {
            ...
            const status = searchResult.values.status.value;
            const customer = searchResult.values.customer.value;
            const subscriptionId = searchResult.id;
        ...
        const reduce = (reduceContext) => {
            const authorUser = runtime.getCurrentUser();
            const authorId = authorUser.id;
            const customerId = reduceContext.key;

            for (let i = 0; i < reduceContext.values.length; i++) {
                const customerSub = JSON.parse(reduceContext.values[i]);
                log.debug('sub ' + customerSub.subscriptionId + ' : ', reduceContext.values[i]);
              ...
              /*  const customerRecord = record.load({
                ...
                const customerName = customerRecord.getValue('companyname');
                ...
                try {
                    email.send({
                        ...
                        body: `Dear customer, "(${customerName})" you need to update your subscription IMMEDIATELY!!!`,
